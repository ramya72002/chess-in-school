...
const SignIn = () => {
  ...
  const signIn = async (emailToSignIn: string) => {
    ...
    try {
      ...
      const loginResponse = await axios.post('http://127.0.0.1:80/signin_inschool', {
        email: emailToSignIn,
      ...
      if (loginResponse.data.success) {
        ...
        const userDetailsResponse = await axios.get('http://127.0.0.1:80/getinschooldetails', {
      ...
      setMessageText('Email is not registered to this portal please connect Admin');
    } finally {
  ...
  const handleLogoutFromPreviousDevice = async () => {
    if (email) {
      try {
        ...
        const response = await axios.post('http://127.0.0.1:80/delete_session_inschool', { email });
  ...
  const verifyOtp = async () => {
    setLoading(true); // Start loading
    try {
      const verifyOtpResponse = await axios.post('http://127.0.0.1:80/verify_otp_inschool', { email, otp });
